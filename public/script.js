...
// ⚡ Bolt: Cache Intl.DateTimeFormat instances to avoid costly initialization on every update
const currentWeatherDateFormatter = new Intl.DateTimeFormat("en-US", {
  weekday: "long",
  hour: "2-digit",
  minute: "2-digit",
});

const weeklyForecastDateFormatter = new Intl.DateTimeFormat("en-US", {
  weekday: "short",
});
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateCurrentWeather(current, today, tempUnit, data) {
    ...
    currentDatetimeEl.textContent = currentWeatherDateFormatter.format(new Date());
  ...
  function updateWeeklyForecast(data, tempUnit) {
    ...
    displayDays.forEach((day, index) => {
      ...
      card.innerHTML = `
                <p class="font-bold text-gray-500 dark:text-gray-400 text-sm mb-2">${index === 0 ? "Today" : weeklyForecastDateFormatter.format(date)}</p>
